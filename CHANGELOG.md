...
- **Elite vs Standard Moveset Analysis**: Expanded PvE moveset optimizer to evaluate and display both the absolute best (Elite/Legacy) moveset (indicated by a `*` suffix) and the best standard moveset (obtainable via standard TMs). If the user possesses the best standard moveset, it is marked as green (with a `(Std)` tag) while still showing the Elite upgrade option.
- **Fuzzy Species Lookup**: Implemented a recursive prefix-matching search for database queries, allowing the moveset engine to find correct data for special species forms (e.g. `Zacian (Crowned Sword)` and `Dialga (Origin)` now retrieve their correct movesets from `POKEMON_DB`).
